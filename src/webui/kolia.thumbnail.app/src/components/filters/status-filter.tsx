...
export const statusFilterOptions: {
  ...
  { value: 'all', label: 'Tất cả', dotClassName: 'bg-slate-400' },
...
export function StatusFilterGroup({ value, onChange }: StatusFilterGroupProps) {
  ...
  return (
    <div>
      ...
      <div className="relative flex rounded-lg bg-slate-100 p-0.5 ring-1 ring-inset ring-slate-200/80">
        {/* Sliding indicator — width/left derived from the same padding box as the buttons, so it always lines up exactly */}
        <div
          className="absolute inset-y-0.5 left-0.5 rounded-md bg-white shadow-[0_1px_2px_rgba(15,23,42,0.08)] ring-1 ring-slate-200/60 transition-transform duration-300 ease-out"
          style={{
