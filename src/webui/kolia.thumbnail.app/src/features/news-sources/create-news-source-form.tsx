...
import { createNewsSource } from './api'
import { createNewsSourceSchema, type CreateNewsSourceInput } from './schema'
...
interface CreateNewsSourceFormProps {
  onClose?: () => void
}

export interface CreateNewsSourceFormHandle {
  submit: () => Promise<void>
  isSubmitting: boolean
}

export const CreateNewsSourceForm = forwardRef<CreateNewsSourceFormHandle, CreateNewsSourceFormProps>(
  ({ onClose }, ref) => {
    ...
    const {
      ...
      setValue,
      watch,
    } = useForm<CreateNewsSourceInput>({
      resolver: zodResolver(createNewsSourceSchema),
      defaultValues: {
        name: '',
        rssOrFeedUrl: '',
        region: 1,
        isTrusted: true,
        priority: 0,
        sourceGroup: undefined as unknown as number,
        fetchMode: 1,
        domain: '',
      },
    })
    ...
    const { mutateAsync } = useMutation({
      mutationFn: (data: CreateNewsSourceInput) => createNewsSource(data),
      onError: (error) => {
        if (error instanceof ApiError && error.isValidationError) {
          error.errors?.forEach((validationError) => {
            setError(validationError.property as keyof CreateNewsSourceInput, {
    ...
    const onSubmit = async (data: CreateNewsSourceInput) => {
      setIsSubmitting(true)
      try {
        await mutateAsync(data)
        toast.success('Tạo nguồn tin thành công!')
      } finally {
...
CreateNewsSourceForm.displayName = 'CreateNewsSourceForm'
