...
export function SocialMediaProvidersPage() {
  ...
  const [deleteTarget, setDeleteTarget] = useState<SocialMediaProviderBaseDto | null>(null)

  const { mutate: remove, isPending: isDeleting } = useMutation({
    mutationFn: (id: string) => deleteSocialMediaProvider(id),
    onSuccess: () => {
  ...
  const { data, isLoading, error, refetch } = useQuery({
    ...
    queryFn: () =>
      getSocialMediaProvidersWithPaging({
        pageNumber: page,
        pageSize: pageSize,
  ...
  const columns = useMemo(
    () => [
      { key: 'name', header: 'Tên', sortable: true, render: (item: SocialMediaProviderBaseDto) => item.name },
      { key: 'shortName', header: 'Mã', sortable: true, render: (item: SocialMediaProviderBaseDto) => item.shortName },
      {
        key: 'baseUrl',
        header: 'Base URL',
        render: (item: SocialMediaProviderBaseDto) => (
      ...
      },
      {
        ...
        sortable: true,
        render: (item: SocialMediaProviderBaseDto) => {
          const label = getSocialMediaProviderTypeLabel(item.providerType as CSocialMediaProviderType)
          return (
            <span className={`inline-flex items-center rounded-full px-2.5 py-0.5 text-xs font-medium ${getSocialMediaProviderTypeBadgeClass(item.providerType)}`}>
      ...
      },
      {
        ...
        header: 'Tạo lúc',
        sortable: true,
        render: (item: SocialMediaProviderBaseDto) => formatDateTime(item.creationTime),
      },
      {
        key: 'lastModificationTime',
        header: 'Cập nhật lúc',
        sortable: true,
        render: (item: SocialMediaProviderBaseDto) => formatDateTime(item.lastModificationTime),
      },
      {
        key: 'actions',
        header: '',
          ...
          item.isDeleted ? (
            ...
            <div className="flex items-center gap-0.5">
              <Button
                variant="ghost"
                size="icon"
                onClick={() => open({ type: 'edit-social-media-provider', provider: item })}
                title="Chỉnh sửa"
              >
    ...
    <div className="space-y-5">
      ...
      <div>
        ...
        <SelectDropdown<{ id: number; label: string }>
          ...
          value={SOCIAL_MEDIA_PROVIDER_TYPE_OPTIONS.find((opt) => String(opt.id) === filterProviderType) ?? null}
  ...
  return (
    <div className="mx-auto max-w-7xl">
      <DataTable
        ...
        // ── Toolbar ─────────────────────────────────────────────
        actions={
          <Button onClick={() => open({ type: 'create-social-media-provider' })} className="gap-2">
